...
class LocalWalkerController extends MetaformaRuntime implements ControllerInformationProvider {	
	...
	enum StateOperation implements IStateOperation {
		DEFAULT, GETUP, GETDOWN, GETDOWN_CHECK;
	...
	void gradientPropagate (boolean isSource, byte var) {
		...
		if (isSource) {
			...
			var = min(MAX_BYTE,var + 1);
		}
		
		broadcast(callback,var);
	}
...
//					rotate(90);
	...
	public void init () {
		...
		setMessageFilter(Type.FIX_SYMMETRY.bit() | Type.STATE_OPERATION_NEW.bit() | Type.GRADIENT.bit());
	...
	protected void receiveMessage(Packet p, int connector) {
		...
		void callback (byte valRec) {
			var = min(var,valRec);
		}
		
		if (p.getType() == Type.GRADIENT && varGet(p.getData()[0]) > p.getData()[1]) {
